...
export default function HomePage() {
  ...
  const formatCount = (n: number | undefined) =>
    n != null ? n.toLocaleString(locale) : '0';

  const features = [
    {
      icon: Scale,
      title: t('smartCompare'),
      desc: t('smartCompareDesc'),
      color: 'from-primary-500 to-amber-500',
      link: '/compare',
      linkLabel: t('tryCompare'),
    },
    {
      icon: SlidersHorizontal,
      title: t('smartFilter'),
      desc: t('smartFilterDesc'),
      color: 'from-blue-500 to-cyan-500',
      link: '/cars?sort=aiMatch-asc',
      linkLabel: t('startFilter'),
    },
    {
      icon: Shield,
      title: t('trustedData'),
      desc: t('trustedDataDesc'),
      color: 'from-emerald-500 to-teal-500',
      link: '/cars',
      linkLabel: t('browseCars'),
    },
  ];

  const statCards = [
    { icon: Car, label: t('totalCars'), value: formatCount(stats?.totalCars), color: 'text-primary-400' },
    { icon: Award, label: t('globalBrands'), value: stats?.totalBrands?.toLocaleString(locale) || '0', color: 'text-amber-400' },
    { icon: SlidersHorizontal, label: t('aiRating'), value: t('aleppo'), color: 'text-blue-400' },
    { icon: Star, label: t('avgRating'), value: '4.7', color: 'text-emerald-400' },
  ];
              ...
              {loadingNewest ? (
                ...
                <div className="space-y-3">
                  ...
                  <div className="grid grid-cols-2 gap-3 pt-1">
                    {[
                      { icon: Car, label: t('carSingular'), value: stats?.totalCars || '500+' },
                      { icon: Award, label: t('brands'), value: stats?.totalBrands || '50+' },
        ...
        <div className="page-container">
          <div className="flex items-end justify-between mb-10">
            <div>
              ...
              <p className="section-subtitle">{t('chooseCategory')}</p>
            </div>
          </div>
          ...
          <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
            {categories.map((cat, i) => (
              ...
              >
                ...
                <span className="text-xs text-slate-500 group-hover:text-primary-400 transition-colors">{t('browseNowArrow')}</span>
              </Link>
            ))}
          </div>
        </div>
        ...
        <div className="page-container">
          <div className="flex items-end justify-between mb-10 gap-4">
            <div>
              ...
              <p className="section-subtitle">{t('scrollBrowse')}</p>
            </div>
          ...
          {loadingFeatured ? (
            ...
            <div className="text-center py-16 text-slate-400">{t('noFeaturedCars')}</div>
          ) : (
            <div
              ref={carouselRef}
      ...
      <section className="py-20">
        <div className="page-container">
          <div className="text-center mb-14">
            <h2 className="section-title">{t('whyAutoArabia')}</h2>
            <p className="section-subtitle">{t('whySubtitle')}</p>
          </div>

          <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
            {features.map(({ icon: Icon, title, desc, color, link, linkLabel }) => (
