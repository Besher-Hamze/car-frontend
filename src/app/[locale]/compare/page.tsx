      ...
      <div className="page-container">
        ...
        {result && (
          <div className="space-y-8">
            ...
            {/* Comparison Table */}
            <div>
              <h3 className="text-xl font-bold text-white mb-4">{t('specComparison')}</h3>
              <div className="card overflow-x-auto">
                <table className="w-full">
                  <thead>
                    <tr className="border-b border-dark-700">
                      <th className="text-right text-slate-400 text-sm font-medium p-4 min-w-[140px]">{t('specifications')}</th>
                      {result.cars.map(car => (
                          ...
                          <td key={j} className="p-4 text-center">
                            <div className={clsx(
                              ...
                              {val.value !== undefined && val.value !== null
                                ? `${typeof val.value === 'number' ? val.value.toLocaleString(locale) : val.value} ${field.unit}`
                                : '—'}
                            </div>
                          </td>
                        ))}
