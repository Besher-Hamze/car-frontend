...
export default function SellerTradeDetailPage() {
  ...
  useEffect(() => {
    if (!hydrated) return;
    if (!token) router.replace(`/login?next=/seller/trades/${id}`);
      ...
      <div className="page-container max-w-3xl">
        ...
        {isLoading ? (
          ...
          <TradeRequestDetail
            ...
            onAccept={(ownerPhone) => respondMutation.mutate({ action: 'accept', ownerPhone })}
            onReject={(reason) => respondMutation.mutate({ action: 'reject', reason })}
          />
        )}
      </div>
    </div>
